        ...
        extraLinks: [
            {
                ...
                href: "/apps/cloud-backup-photo-storage/account-deletion/"
            }
        ]
    },
...
(function () {
        ...
        return `
            ...
            >
                <div class="app-card-top">
                    ${appIconMarkup(
                        app,
                        app.icon ? "app-card-icon" : "app-icon-fallback"
                    )}
                    ${statusBadge(app)}
                </div>
                    ...
                    <div class="footer-grid">
                        <div>
                            ...
                            <div class="footer-contact">
                                ...
                                ${
                                    config.address
                                        ? `<span>${escapeHtml(config.address)}</span>`
                                        : ""
                                }
                            </div>
                        </div>
    ...
    function hydrateContactData() {
        ...
        document.querySelectorAll("[data-site-legal-name]").forEach(el => {
            el.textContent = config.legalName || config.brandName || "";
        });
        ...
        document.querySelectorAll("[data-site-logo]").forEach(el => {
            if (config.logo) {
                el.setAttribute("src", config.logo);
            }
        });

        document.querySelectorAll("[data-site-google-play-link]").forEach(el => {
            el.setAttribute("href", config.developerGooglePlayUrl || "#");
        });

        document.querySelectorAll("[data-site-app-store-link]").forEach(el => {
            el.setAttribute("href", config.appStoreLandingUrl || "#");
        });
    }
...
(function () {
    ...
    function renderAppsSlider(direction) {
        ...
        if (sliderNext) {
            sliderNext.disabled = sliderIndex >= maxStart;
        }
    ...
    if (featured) {
        featured.innerHTML = api.featuredMarkup();
    }
            ...
            <div class="container detail-grid">
                ...
                <article class="detail-card">
                    ...
                    <ul class="feature-list">
                        ${(app.features || [])
                            .map(item => `<li>${api.escapeHtml(item)}</li>`)
                            .join("")}
                    </ul>
                </article>
            </div>
